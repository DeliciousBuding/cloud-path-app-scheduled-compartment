...
1. 从同一 Release 下载 `plugin.yaml` 和匹配当前 OS/CPU 的 `cloud-path-app-scheduled-compartment_vX.Y.Z_<os>_<arch>[.exe]`。
2. 使用 Release 中的 `checksums.txt` 校验二进制和 `plugin.yaml`，不要混用不同版本的 manifest 与程序。
3. 通过 CloudPath Core 支持的插件安装流程导入 manifest 与二进制；本程序由 Core Plugin Host 托管，不应作为独立硬件程序直接运行。
4. 启用实例后，按后文的能力契约选择按键、可选显示与可选蜂鸣，并完成实例配置和绑定。

### 0.3.1 可用性修复（相对 v0.3.0 Release）
...
Core 可以先验证候选绑定再下发配置。应用会先保留结构校验通过的绑定，在 `ConfigureInstance` 严格核对格数；配置已存在时，绑定校验同样检查数量一致。两种调用顺序都不会把一键配置静默扩成三格。
